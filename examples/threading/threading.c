...
bool start_thread_obtaining_mutex(pthread_t *thread, pthread_mutex_t *mutex,int wait_to_obtain_ms, int wait_to_release_ms)
{
    ...
    struct thread_data *d = (struct thread_data*)malloc(sizeof(struct thread_data));
    d->wait_to_obtain_ms = wait_to_obtain_ms;
    d->wait_to_release_ms = wait_to_release_ms;
    d->mutex = mutex;
    d->thread_complete_success = false;
    if (pthread_create(thread, NULL, threadfunc, d) != 0) {
        return false;
    }
    return true;
}
